...
void swim_put_diamond(SWIM_WINDOW_T *win,
                      ...
                      INT_32 ry)
{
  INT_32 xinc, idy, xleft, xright, pixel, ypt;

  /* For center line, limit to size of the window */
  xleft = x - rx;
  xright = x + rx;
    if (xleft < 0) xleft = 0;
    if (xright > win->xvsize) xright = win->xvsize;

  /* Draw the left and right pixels of the center line in the pen
     color */
  if(y>=0 && y < win->yvsize)
  {
    *FBXY(win, xleft+win->xpvmin, y+win->ypvmin) =
      *FBXY(win, xright+win->xpvmin, y+win->ypvmin) = win->pen;

  /* Draw the center line first in the fill color */
    for (pixel = (xleft + 1); pixel < (xright - 1); pixel++)
    {
	  *FBXY(win, pixel+win->xpvmin, y+win->ypvmin) = win->fill;
    }
  }
  ...
  for (idy = 1; idy <= ry; idy++)
  {
    ...
    /* Convert virtual coordinates to physical coordinates */
    xleft = xleft + win->xpvmin;
    xright = xright + win->xpvmin;

    /* Only render top line if it is visible */
    if ((y - idy) >= 0 && (y - idy) < win->yvsize)
    {
      /* Convert to physical coordinate */
      ypt = ((y - idy) + win->ypvmin);
      /* Draw the left pixel of the line in the pen color */
	  *FBXY(win, xleft, ypt) = win->pen;

      /* Draw top line in fill color */
      for (pixel = (xleft + 1);
           pixel <= (xright - 1); pixel++)
      {
	    *FBXY(win, pixel, ypt) = win->fill;
      }

      /* Draw the right pixel of the line in the pen color */
	  *FBXY(win, xright, ypt) = win->pen;
    }
